...
class NuscenesVisualizeNode(object):
    ...
    def publish_callback(self):
        ...
        # 1. Get the true timestamp from the nuScenes sample (it's in microseconds)
        sample_timestamp_micros = self.current_sample['timestamp']
        
        # 2. Convert it to seconds
        sample_timestamp_sec = sample_timestamp_micros / 1_000_000.0

        # 3. Create a ROS Time message from the seconds
        ros_time_msg = rclpy.time.Time(
            seconds=int(sample_timestamp_sec),
            nanoseconds=int((sample_timestamp_sec % 1) * 1e9)
        ).to_msg()
        ...
        for i, box in enumerate(bboxes):
            ...
            marker = self.ros_interface.object_to_marker(box, frame_id='LIDAR_TOP', marker_id=i, duration= 1.2 / self.update_frequency, color=obj_color, stamp=ros_time_msg)
...
#         self.bbox_delay_sec = 10.0  # Set your desired delay in seconds
#         self.bbox_buffer = deque()
#         self.ros_interface.create_timer(0.05, self.publish_delayed_bboxes)  # Check buffer every 50ms
...
#         self.published=False
#         self.stop = msg.data #Bool
#         # Clear the bbox buffer when stopping
...
#         self.current_scene = self.nusc.scene[self.index]
#         self.published = False
#         # Clear the bbox buffer when changing scenes
#         self.bbox_buffer.clear()
#         des = self.current_scene["description"]
#         print(f"Switch to scenes {self.index}: {des} ")
#         self.current_sample = self.nusc.get('sample', self.current_scene['first_sample_token']) 
...
#     def pause_callback(self, msg):
#         self.pause = msg.data
#         # Clear bbox buffer when pausing to prevent stale data from being published
#         if self.pause:
#             self.bbox_buffer.clear()
...
#         # Only create and buffer bboxes if not paused and not stopped
...
#             # Add to buffer with delay only if we have markers
#             if markers.markers:
#                 publish_time = self.ros_interface.get_clock().now() + rclpy.duration.Duration(seconds=self.bbox_delay_sec)
#                 self.bbox_buffer.append((publish_time, markers))
...
#     def publish_delayed_bboxes(self):
#         """Publish bboxes that have reached their delay time"""
#         if self.stop:  # Don't publish anything if stopped
#             return
            
#         now = self.ros_interface.get_clock().now()
#         while self.bbox_buffer and self.bbox_buffer[0][0] <= now:
#             _, markers = self.bbox_buffer.popleft()
#             self.ros_interface.publish("/nuscenes/bboxes", markers)
